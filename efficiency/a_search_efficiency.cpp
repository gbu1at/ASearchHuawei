...
CH::AlgorithmEfficiency a_search_efficiency(CH::vertex_t start, CH::vertex_t finish, const CH::Graph &graph,
    ...
    E.result = ASearch::a_search(start, finish, lm, &E.percent, nullptr, &E.cnt_move, &E.cnt_edge_in_way);
...
CH::AlgorithmEfficiency B_a_search_efficiency(CH::vertex_t start, CH::vertex_t finish, const CH::Graph &graph,
    ...
    E.result = BASearch::B_a_search(start, finish, lm, &E.percent, nullptr, &E.cnt_move, &E.cnt_edge_in_way);
...
a_search_landmarks_efficiency(CH::vertex_t start, CH::vertex_t finish, const CH::Graph &graph, CH::LandMarks& lm,
    ...
    lm.set_start_finish(start, finish);

//    std::function<CH::weight_t(CH::vertex_t)> get_dist_s = [&lm, &finish](CH::vertex_t v) {
//        return lm.get_dist(v, finish);
//    };
//
//    std::function<CH::weight_t(CH::vertex_t)> get_dist_f = [&lm, &start](CH::vertex_t v) {
//        return lm.get_dist(v, start);
//    };


    CH::AlgorithmEfficiency E;
    if (!is_B_search) {
        E = a_search_efficiency(start, finish, graph, lm);
        E.comment = "a search + landmarks cnt: " + std::to_string(lm.cnt_landmarks);
//        for (auto &x : lm_s.mp)
//            std::cout << x.first << " " << x.second << "\n";
    } else {
        E = B_a_search_efficiency(start, finish, graph, lm);
